...
def sample_examples(docs,max_window_size,n_windows):
    ...
    for i,doc in enumerate(docs):
        ...
        sub_samples = []
        for sample in get_windows(doc, window_size):
            sub_samples.append(sample)
        windows.append(sub_samples)
...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    ...
    ### fill the gaps ### 
    term_pos = - Wc[pos,] * (1-factors_pos).reshape(-1, 1)
    term_negs = Wc[negs,] * (1-factors_negs).reshape(-1, 1)
    partials_pos = - (1-factors_pos).reshape(-1,1) @ Wt[target,].reshape(1,-1)
    partials_negs = (1-factors_negs).reshape(-1,1) @ Wt[target,].reshape(1,-1)
...
if not train:
    ...
    words_array_1 = ['banana','watch','movie']
    words_array_2 = ['bread','listen','film']
    try:
        id_words_array_1 = []
        id_words_array_2 = []
        for wrd in words_array_1:
            id_words_array_1.append(vocab[wrd])
        for wrd in words_array_2:
            id_words_array_2.append(vocab[wrd])
        
        cos_sim = cosine(Wc[id_words_array_1,],Wc[id_words_array_2,])
        
        for i,w1 in enumerate(words_array_1):
            for j,w2 in enumerate(words_array_2):
                print("Cosine similarity of '"+w1+"' and '"+w2+"' is:",cos_sim[i,j])
        
    except KeyError:
        print("The word '"+wrd+"' is not in vocaburary.")
